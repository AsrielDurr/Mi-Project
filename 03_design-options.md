...
|   |-- src/
|   |   |-- modules/
|   |   |   |-- recommendation/       # M1：推荐前端组件
|   |   |   |-- enrollment/           # M2：规则结果与选课组件
|   |   |   `-- waitlist/             # M3：候补与追溯组件
|   |   |-- shared/                   # 由共享契约生成或冻结
|   |   `-- integration/
|   |       `-- App.vue               # 最终仅做组件组合与事件转发
|   |-- tests/
|   |   |-- recommendation/
|   |   |-- enrollment/
|   |   `-- waitlist/
...
|   |-- app/
|   |   |-- main.py
|   |   |-- contracts/                # Pydantic模型与Ports
|   |   |-- modules/
|   |   |   |-- recommendation/       # M1：MiMo、校验、推荐Router
|   |   |   |-- enrollment/           # M2：规则、状态决策Router
|   |   |   `-- waitlist/             # M3：Store、候补、追溯Router
|   |   `-- integration/
|   |       `-- bootstrap.py          # 最终只做依赖注入
|   |-- tests/
|   |   |-- recommendation/
|   |   |-- enrollment/
|   |   `-- waitlist/
...
- M1独占前后端`modules/recommendation/`及对应测试，交付可独立运行的AI推荐纵向切片。
- M2独占前后端`modules/enrollment/`及对应测试，交付可独立运行的规则与选课纵向切片。
- M3独占前后端`modules/waitlist/`及对应测试，交付可独立运行的候补、状态和追溯纵向切片。
- `contracts/`、前后端共享类型和模拟数据在并行开发前由三人共同冻结。
- 三个模块只依赖共享Ports和事件，不直接导入彼此实现；最终由`bootstrap.py`和`App.vue`接线。
...
- [√] 已发散5个不同技术方案。
- [√] 已筛选3个方案进行成本、效果和具体风险比较。
- [√] 已推荐1个最终方案并说明理由。
- [√] 已定义模块结构、职责边界和禁止行为。
- [√] 已定义数据事实来源、状态枚举和状态转换。
- [√] 已提供Happy Path和至少3条Failure Path。
- [√] 已定义模型校验、候补重算和演示重置策略。
- [√] 已给出最小接口契约和响应示例。
- [√] 人工已批准采用S2。
- [√] 人工已确认大模型供应商为MiMo。
- [√] 人工已确认具体模型名称。
- [√] 人工已接受内存状态在进程重启后丢失。
- [√] 人工已确认允许fallback作为故障备用，但不能替代真实MiMo主流程。

## 14. 人工确认记录

| 确认项 | 结论 | 确认人 | 日期 | 备注 |
|---|---|---|---|---|
| 5→3筛选 | 通过 | 用户 | 2026-07-16 | 排除浏览器直连模型和LLM统一决策方案 |
| 最终选择S2 | 批准 | 用户 | 2026-07-16 | Vue双角色前端+FastAPI单体后端+内存状态 |
| 成本/效果/风险矩阵 | 待确认 | — | 2026-07-16 | 确认评分和一天工期判断合理 |
| 大模型供应商与模型 | 部分确认 | 用户 | 2026-07-16 | 供应商采用MiMo；具体模型名称和环境变量名待补充 |
| 状态存储方式 | 通过 | 用户 | 2026-07-16 | 接受JSON种子数据+内存Store，接受进程重启后状态丢失 |
| fallback演示策略 | 通过 | 用户 | 2026-07-16 | 允许作为MiMo异常时的故障备用；必须标记来源，不得替代真实模型主流程 |
| Failure Paths | 待确认 | — | 2026-07-16 | 覆盖空输入、模型失败、虚构课程、冲突、候补失效和状态污染 |
| 是否进入开发流程文档 | 待批准 | — | 2026-07-16 | 方案确认后进入`04_dev-workflow.md` |
