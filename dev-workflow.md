...
* [ ] 上述5类领域对象已写入Schema并通过样例校验。

* [ ] 枚举与`product-prd.md`完全一致。
...
* [x] Step 0共享契约已实际创建并提交。

* [x] 具体MiMo模型名称和调用配置已确认。
...
| 确认项        | 结论          | 日期         | 备注                                                                                      |
| ---------- | ----------- | ---------- | --------------------------------------------------------------------------------------- |
| 能力模块拆分     | 已确认         | 2026-07-16 | 已确认按纵向业务能力拆分为M1 AI推荐、M2规则与选课、M3候补重算与追溯；每个模块均包含必要的前端、后端、测试和独立Demo。                       |
| 共享Ports与事件 | 已确认方案，待实际冻结 | 2026-07-16 | 已确认三个模块只依赖共享Schema、枚举、Ports和前端事件，不直接导入彼此实现；具体契约文件仍需在Step 0中实际创建、校验并提交。                  |
| 独立Demo策略   | 已确认         | 2026-07-16 | 已确认M1、M2、M3分别使用Fake或Stub依赖独立运行和测试，独立Demo通过后才允许进入合并阶段。                                   |
| 真实组合策略     | 已确认         | 2026-07-16 | 已确认由`bootstrap.py`注入真实`InMemoryStore`、`RuleEngine`和MiMo适配器，正式组合入口只负责依赖注入和组件接线，不新增业务逻辑。  |
| 合并顺序       | 已确认         | 2026-07-16 | 已确认开发阶段三个模块并行，合并阶段按M3→M2→M1执行，最后连接后端Ports和前端事件，并清除正式入口中的Fake、Stub和静态Mock。               |
| V1—V4集成门禁  | 已确认         | 2026-07-16 | 已确认使用正常推荐并选课成功、时间冲突拒绝、满员进入候补、第一名失效后第二名补入四个场景验证三模块完整闭环；真实MiMo成功调用不得由fallback替代。          |
| 是否允许进入编码   | 已确认         | 2026-07-16 | 开发流程方案已确认，但正式编码前仍需完成`05_test-strategy.md`、质量门禁文档、Step 0共享契约的实际创建与提交，并确认具体MiMo模型名称和调用配置。 |
